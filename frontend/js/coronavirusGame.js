function isConnected(){
    ...
    if (ifConnected == false) {
		localStorage.setItem("lastWindow", "coronavirusGame.html")
...
function goBack() {
	...
	fetch("http://localhost:80/api/score", {
		...
		console.log("erro ao guardar score");
	});
	...
	if (!gameover) {
		...
		if (danger) {
			...
			if (health < 10) {
				...
				ctx.fillText("SAÚDE BAIXA", stage.w / 2, stage.h * 0.6);
			}
		} else {
			dangera = 0;
		}
		...
		ctx.fillText("Pontos: " + score, stage.w - 200, 40);
		...
		ctx.textAlign = "center";
		ctx.textBaseline = "middle";
		ctx.fillText("PERDESTE!", stage.w / 2, stage.h / 3);
		ctx.font = "bold 50px arial";
		ctx.fillText("Pontuação: " + score, stage.w / 2, stage.h / 2);

		ctx.font = "bold 30px arial";

		ctx.fillText("JOGAR MAIS", stage.w / 2, stage.h * 0.65);
			...
			if (can.x > (stage.w - 220) / 2 && can.y > stage.h * 0.65 - 25 && can.x < (stage.w - 220) / 2 + 220 && can.y < stage.h * 0.65 - 25 + 50) {
				...
				if (againprog > 220) {
					if (score > biggestscore) {
						document.getElementById("bestscore").innerHTML = "Melhor Pontuação: " + score;
					}
					newGame();
					gameover = false;
				}
